					...
					<div class="single-event-content">
						...
						<section class="event-time">
							...
							<p><?php
								...
								echo MakespaceFrameworkEventsModule::get_event_times(get_the_ID());
							?></p>
						</section>
						...
						<?php if(get_field('location') || get_field('location_name')): ?>
							<section class="event-location">
								...
								<p>
									...
									<div id="gmap" data-gmapSingleLocation="<?php the_ID(); ?>" data-lat="<?php echo $event_location['lat']; ?>" data-lng="<?php echo $event_location['lng']; ?>" data-maxZoom="18" data-minZoom="1">
										<?php 
											$location_info = '';
											$address_name = $event_location['name'];
											$address_street_number = $event_location['street_number'];
											$address_street_name = $event_location['street_name_short'];
											$address_city = $event_location['city'];
											$address_state = $event_location['state_short'];
											$address_zip = $event_location['post_code'];
											$address_country = $event_location['country'];

											if($address_name){
												$location_info .= '<h3 class="map-location-name">' . $address_name . '</h3>';
											}
											if($address_street_number){
												$location_info .= $address_street_number . ' ';
											}
											if($address_street_name){
												$location_info .= $address_street_name . '<br>';
											}
											if($address_city){
												$location_info .= $address_city . ', ';
											}
											if($address_state){
												$location_info .= $address_state;
											}
											if($address_zip){
												$location_info .= ' ' . $address_zip;
											}
											if($address_country !== 'United States'){
												$location_info .= ', ' . $address_country;
											}
										 ?>
										<div class="infowindow"><?php echo $location_info; ?></div>
									</div>
									...
									?>
										<a href="<?php echo $directions_url_google; ?>" target="_blank">Get Directions</a>
