(function($){
		...
		if( MSWObject.google_map_data ){
			...
			if( document.getElementById( 'gmap' ) ){
				...
				if(document.querySelector('[data-gmapSingleLocation]') ){
					var locationEl = document.querySelector('[data-gmapSingleLocation]');
					locationId = locationEl.getAttribute('data-gmapSingleLocation');
					locationLat = locationEl.getAttribute('data-lat');
					locationLng = locationEl.getAttribute('data-lng');
					locationInfo = $(locationEl).find('.infowindow').html();
					// singleMarker.push(markers.find(markerMatchesLocationId));
					var locationData = {
						lat: locationLat,
						lng: locationLng,
						infowindow_content: locationInfo
					};
					singleMarker.push(locationData);
	...
	var filterDropdown = function(){
		...
		$(document).on('click', '.filter-dropdown a', function(e){
			e.stopPropagation();
